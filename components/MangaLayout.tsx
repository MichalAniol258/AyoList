...
const MediaPage = dynamic(() => import("@/src/app/(main)/manga/[id]/mediaPage"), {
    ssr: false,
    loading: () => null,
});
...
export default function PMangalist({ params: { id } }: PMangalistProps) {
    ...
    const { data: userList2, loading: userLoading2, } = useQuery(MEDIA_INFO, {
        variables: {
            mediaId: id, language: "JAPANESE", userId: userInfo?.id, mediaListCollectionUserId2: userInfo?.id, type: "MANGA", isMain: true, version: 2, role: undefined, sort: "ID", staffSort2: "RELEVANCE", recommendationsSort2: 'RATING_DESC',
    ...
    const { data: userList, loading: userLoading, fetchMore } = useQuery(MEDIA_INFO, {
        variables: {
            page: 1,
            perPage: 20,
            ...
            staffPage2: 1,
            staffPerPage2: 20, mediaId: id, language: selectedValue, userId: userInfo?.id, mediaTrendMediaId2: id, mediaListCollectionUserId2: userList2?.Following?.id, type: "MANGA", isMain: true, version: 2, role: undefined, sort: "ID", staffSort2: "RELEVANCE", recommendationsSort2: 'RATING_DESC',
    ...
    }, [userList, userLoading, fetchMore]);
    ...
    useEffect(() => {
        const target = endOfListRef.current;
        if (!target || !pathname.includes("/characters")) return;
        ...
        return () => {
            observer.unobserve(target); // używaj lokalnej kopii
        };
    }, [pathname, loadMore]);
    ...
    useEffect(() => {
        const target = endOfListRef3.current;
        if (!target || !pathname.includes("/characters")) return;
    ...
    }, [pathname, loadMoreStaff]);



    useEffect(() => {
        const target = endOfListRef2.current;
        if (!target || !pathname.includes("/characters")) return;
    ...
    }, [pathname, loadMoreRecommendations]);
        ...
        <>
            ...
            <div className="contentContainer content-layout">
                                    ...
                                    >
                                        <div className="icon-wrap" style={{
                                            ...
                                            {!item.icon ? <svg data-v-c1b7ee7c="" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="link" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="icon-media2"><path data-v-c1b7ee7c="" fill="currentColor" d="M326.612 185.391c59.747 59.809 58.927 155.698.36 214.59-.11.12-.24.25-.36.37l-67.2 67.2c-59.27 59.27-155.699 59.262-214.96 0-59.27-59.26-59.27-155.7 0-214.96l37.106-37.106c9.84-9.84 26.786-3.3 27.294 10.606.648 17.722 3.826 35.527 9.69 52.721 1.986 5.822.567 12.262-3.783 16.612l-13.087 13.087c-28.026 28.026-28.905 73.66-1.155 101.96 28.024 28.579 74.086 28.749 102.325.51l67.2-67.19c28.191-28.191 28.073-73.757 0-101.83-3.701-3.694-7.429-6.564-10.341-8.569a16.037 16.037 0 0 1-6.947-12.606c-.396-10.567 3.348-21.456 11.698-29.806l21.054-21.055c5.521-5.521 14.182-6.199 20.584-1.731a152.482 152.482 0 0 1 20.522 17.197zM467.547 44.449c-59.261-59.262-155.69-59.27-214.96 0l-67.2 67.2c-.12.12-.25.25-.36.37-58.566 58.892-59.387 154.781.36 214.59a152.454 152.454 0 0 0 20.521 17.196c6.402 4.468 15.064 3.789 20.584-1.731l21.054-21.055c8.35-8.35 12.094-19.239 11.698-29.806a16.037 16.037 0 0 0-6.947-12.606c-2.912-2.005-6.64-4.875-10.341-8.569-28.073-28.073-28.191-73.639 0-101.83l67.2-67.19c28.239-28.239 74.3-28.069 102.325.51 27.75 28.3 26.872 73.934-1.155 101.96l-13.087 13.087c-4.35 4.35-5.769 10.79-3.783 16.612 5.864 17.194 9.042 34.999 9.69 52.721.509 13.906 17.454 20.446 27.294 10.606l37.106-37.106c59.271-59.259 59.271-155.699.001-214.959z"></path></svg>
                                                : <Image fill sizes="(max-width: 768px) 50vw, (max-width: 1200px) 25vw, 20vw" src={`${item.icon}`} alt="cos" className="icon object-cover" />}
                                        </div>
                                        <span className="name">
                      {item.site}
                ...
                <div className="Overview">
                    {pathname === `/manga/${id}` && <>    <div className='description-wrap'>
                        <h2>Description</h2>
                    ...
                    {(pathname === `/manga/${id}` || pathname.includes('/characters')) && <div className="characters">
                        ...
                        <div className="grid-wrap">
                            ...
                            {pathname === `/manga/${id}` && media3.characters?.edges?.slice(0, 6).map((edge, index) => {
                    ...
                    {(pathname === `/manga/${id}` || pathname.includes('/staff')) && media.staff?.edges.length > 0 && <div className="staff">
                    ...
                    {(pathname === `/manga/${id}` || pathname.includes("/stats")) && media.stats?.statusDistribution.length > 0 && <div className="grid-section-wrap">
                    ...
                    {(pathname === `/manga/${id}` || pathname.includes('/watch')) && media.streamingEpisodes?.length > 0 && <div className="Episodes">
                    ...
                    {pathname === `/manga/${id}` && filteredMediaIds?.length > 0 && <div className="grid-section-wrap">
                        <div className="">
                    ...
                    {pathname === `/manga/${id}` && media.recommendations?.edges.length > 0 && <div className="recommendations">
                    ...
                    {media.tags?.length > 0 && <div className="tags max-md:block md:hidden">
                        <h2>Tags</h2>
                        {media.tags
                            ?.filter((entry) => !entry.isMediaSpoiler || isSpoiler) // Jeśli isSpoiler == true, pokazuje oba rodzaje tagów
                            .map((item, index) => (
                    ...
                    {media.externalLinks?.length > 0 && <div className="external-links max-md:block md:hidden">
                        <h2>External & Streaming links</h2>
                        <div className="external-links-wrap">
                            {media.externalLinks?.map((item, index) => {
                                const language = item.language ? item.language.toLowerCase() : '';
