...
function AnimeSeasonComponent() {
    ...
    // Memoizacja mediów - pokazuj wszystkie
    const displayedMedia = useMemo(() => {
        return media || [];
    }, [media]);

    // Early return dla błędu
    if (errorSeason) return <p>Something went wrong: {errorSeason.message}</p>;
        ...
        <section className="sMain">
            <main>
                <div className="showAll">
                    <h2>
                        <span>{season} </span>{seasonYear} Anime
                    </h2>
                    <Link href="/Browse/seasonalAll">
...
const AnimeSeason = memo(AnimeSeasonComponent);
